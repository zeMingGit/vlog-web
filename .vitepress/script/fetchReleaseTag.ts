export const fetchReleaseTag = () => {
  return fetch('https://api.github.com/repos/zeMingGit/vlog/releases/latest')
    ...
    .then((releaseTag) => {
      ...
      const sampDoc = document.querySelector(
        'div.VPHero.has-image.VPHomeHero > div > div.main > p.tagline > .docs-cn-github-release-tag',
      )
      const docsReleaseTagSpan = document.createElement('samp')
      docsReleaseTagSpan.classList.add('docs-cn-github-release-tag')
      docsReleaseTagSpan.innerText = releaseTag || 'v1.0.0'
      if (!sampDoc) {
